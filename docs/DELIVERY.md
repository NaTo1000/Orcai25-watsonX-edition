...
6. verifies and records the published digest.
...
A non-prerelease GitHub release promotes `stable`. Manual dispatch only
promotes it when `promote_stable` is selected.

## Standalone Distribution

Download the `orcai25-<version>-linux-amd64-docker` workflow artifact, then:
...
The archive is retained for 30 days by the workflow unless repository policy
overrides or removes it. Copy approved release artifacts to the organization's
long-term distribution store when longer retention is required.
...
1. Identify a known-good version, `sha-<commit>` tag, or `sha256:<digest>`.
